...
export default defineComponent(() => {
  ...
  const messageDom = () =>
    msgList.length > 0 && (
      <div
        class="chat-wrapper"
        style={{ marginBottom: chatBottomHeight.value + 50 + 'px' }}>
        {msgList.map((item) => (
          <li
            class={[
              'flex pd10 align-start',
            ...
            <div
              class={[
                'mg-l10 bubble',
                item.direction === 'left' ? 'bubble-left' : 'bubble-right',
              ]}>
  ...
  const answerDom = () =>
    ansList.length > 0 ? (
      <div class="answers-wrapper flex flex-wrap max640" ref={answerRef}>
        {ansList.map((item: string) => (
          <div class="btnbox col-6 flex">
            <div class="flex flex-center btn" onClick={() => addAnswer(item)}>
              {answerList[item]}
            </div>
          </div>
        ))}
      ...
      <div class="answers-wrapper max640">
        <div
          class="loadingtxt fs12 lh20  mg10 text-center"
