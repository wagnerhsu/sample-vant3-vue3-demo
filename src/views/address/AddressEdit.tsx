    ...
    const handleDelete = () => {
      Dialog.confirm({
        ...
        .then(() => {
          ...
          localAddress = localAddress?.filter(
            (v: AddressInfo) => v.id !== form.id
          );
          if (form.isDefault) {
            if (Array.isArray(localAddress) && localAddress.length > 0)
              localAddress[0].isDefault = true;
          }
